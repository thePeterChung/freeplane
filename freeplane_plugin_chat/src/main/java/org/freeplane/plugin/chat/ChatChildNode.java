...
class ChatChildNode extends AFreeplaneAction {
	...
	public void actionPerformed(final ActionEvent e) {
		...
		if (found != null) {
			...
			int childCount = nodeModel.getChildCount();
			nodeModel.children();
			List<NodeModel> nodeChild = nodeModel.getTheChildren();
			for (int i = 0; i < nodeChild.size(); i++) {
					Vector<Attribute> childNodeAttributes = chatHelper.getAttributeTableFromNode( nodeChild.get(i) );
					if (!childNodeAttributes.isEmpty()) {
						context.addAll(chatContext.createContext(childNodeAttributes));
					}
			}
